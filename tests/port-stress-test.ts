...
import { PortRegistryManager } from '../src/utils/port-registry';
...
async function testConcurrentAllocations() {
  ...
  try {
    ...
    console.log('Ports in registry:', registry.instances.map((i: any) => i.port).sort((a: number,b: number) => a-b).join(', '));
  } catch (err: any) {
  ...
  try {
    ...
    if (registry.instances.length === 0 || registry.instances.length === 1) {
      // Allow 1 instance (the main running server)
      console.log('✅ Test instances cleaned up successfully\n');
    } else {
      console.warn('⚠️  Some instances remain:', registry.instances.map((i: any) => i.port));
    }
  } catch (err: any) {
