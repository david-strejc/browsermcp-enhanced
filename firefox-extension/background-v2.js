...
async function handleSelectOption(tabId, { ref, element, values }) { try { if (typeof tabId!=='number') return { success:false, error:'No active tab' }; return await browserAPI.tabs.sendMessage(tabId, { action:'select', ref, element, values }); } catch(e){ return { success:false, error:String(e) }; } }
...
async function handleExecuteJS(tabId, { operation, params={} }) { try { if (typeof tabId!=='number') return { success:false, error:'No active tab' }; const safe=['getText','getValue','getAttribute','exists','isVisible','getPageInfo','extractTable','extractLinks','extractImages','scrollTo','highlight','getComputedStyle']; if (!safe.includes(operation)) return { success:false, error:`Operation '${operation}' not allowed. Only predefined safe operations are permitted.` }; return await browserAPI.tabs.sendMessage(tabId, { action:'executeSafeOperation', operation, params }); } catch(e){ return { success:false, error:String(e) }; } }
